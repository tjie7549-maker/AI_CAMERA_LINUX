...
static void *ai_cam_forward_frames(void *arg) {
	...
	while (!ai_cam_is_stopping(app)) {
...

		{
			...
			frame_count++;
			if (display_frame.stVFrame.u64PTS > 0 && now_us >= display_frame.stVFrame.u64PTS) {
			...
			if (now_us - stat_start_us >= 1000000) {
				printf("#Stats: LCD FPS=%.2f, capture-to-display avg=%.2f ms, max=%.2f ms\n",
