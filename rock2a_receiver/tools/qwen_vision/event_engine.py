"""Hardware-independent object event state machine."""
...
class EventEngine:
    def __init__(self, end_grace_seconds: int = 10, stale_seconds: int = 5,
                 initial_delay_seconds: int = 3, cooldown_seconds: int = 120,
                 long_refresh_seconds: int = 300,
                 scene_change_threshold: float = 0.15) -> None:
        self.events: dict[str, Event] = {}
        self.active_by_camera: dict[str, str] = {}
        self.end_grace_ms, self.stale_ms = end_grace_seconds * 1000, stale_seconds * 1000
        self.initial_delay_ms, self.cooldown_ms = initial_delay_seconds * 1000, cooldown_seconds * 1000
        self.long_refresh_ms = long_refresh_seconds * 1000
        self.scene_change_threshold = scene_change_threshold
        self.messages_total = self.messages_invalid = 0
        self.last_message_by_camera: dict[str, int] = {}

    def _event_message(self, event: Event, kind: str, frame_id: int = 0, captured_at_ms: int = 0) -> dict[str, Any]:
        best = event.best
        return envelope(kind, event.camera_id, frame_id=frame_id, captured_at_ms=captured_at_ms,
                        event_id=event.event_id, tracks=list(event.tracks.values()),
                        result={"event_state": event.state, "current_people": event.current_people,
                                "max_people": event.max_people, "track_count": len(event.tracks),
                                ...
                                "frame_match": "approximate", "cloud_state": event.cloud_state,
                                "warning": event.warning, "warning_reason": event.warning_reason,
                                "summary": event.summary})

    def ingest(self, message: dict[str, Any], candidate: Any = None, now_ms: int | None = None) -> list[dict[str, Any]]:
        ...
        if now - received > self.stale_ms:
            ...
            return [envelope("health", message["camera_id"],
                             health={"status": "stale_detection",
                                     "age_ms": now - received})]
        ...
        if created:
            ...
            event = Event(event_id, camera_id, now, state="new", last_seen_at=now,
                          last_update_at=now)
            self.events[event_id] = event; self.active_by_camera[camera_id] = event_id
        if event is None: return output
        if tracks:
            previous_people = event.current_people
            changed = event.state != "active" or {item["track_id"] for item in tracks} != set(event.tracks)
            event.ending_since, event.last_seen_at = None, now
            event.tracks = {item["track_id"]: item for item in tracks}
            event.all_tracks.update(event.tracks)
            event.current_people = len(tracks); event.max_people = max(event.max_people, event.current_people)
            if previous_people != event.current_people:
                event.cloud_pending = True
            if created:
                output.append(self._event_message(event, "event.new", message["frame_id"], captured))
                event.state = "active"
            elif changed:
                event.state = "active"
            ...
            event.state, event.current_people, event.ending_since = "ending", 0, now
            event.last_update_at = now
            output.append(self._event_message(event, "event.update", message["frame_id"], captured))
        return output
    ...
    def tick(self, now_ms: int | None = None) -> list[dict[str, Any]]:
        ...
        for camera_id, event_id in list(self.active_by_camera.items()):
            ...
            if event.state in {"active", "ending"} and now - last_message >= self.stale_ms:
                event.state = "interrupted"; event.last_seen_at = now; event.current_people = 0
                output.append(self._event_message(event, "event.end"))
                del self.active_by_camera[camera_id]
                continue
            if event.state == "ending" and event.ending_since is not None and now - event.ending_since >= self.end_grace_ms:
                event.state = "ended"; event.last_seen_at = now
                output.append(self._event_message(event, "event.end"))
                del self.active_by_camera[camera_id]
                continue
    ...
    def due_cloud_events(self, now_ms: int | None = None) -> list[Event]:
        ...
        for event_id in self.active_by_camera.values():
            event = self.events[event_id]
            if event.cloud_inflight or not event.best: continue
            age = now - event.started_at
            initial = event.last_cloud_at == 0 and age >= self.initial_delay_ms
            changed = event.last_cloud_at > 0 and event.cloud_pending and now - event.last_cloud_at >= self.cooldown_ms
            refresh = event.last_cloud_at > 0 and now - event.last_cloud_at >= self.long_refresh_ms
            if initial or changed or refresh:
                due.append(event); event.cloud_inflight = True; event.cloud_pending = False; event.cloud_state = "running"
        return due
