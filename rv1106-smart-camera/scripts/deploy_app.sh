...
usage() { echo "Usage: $0 [--env BOARD_ENV] [--ip BOARD_IP] [--user root] [--config FILE] [--dry-run]" >&2; exit 2; }
project_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
build_dir=${BUILD_DIR:-$project_dir/build}
board_ip=${BOARD_IP:-} user=${BOARD_USER:-root} dry_run=false
...
for f in "$build_dir/bin/media-sender" "$build_dir/bin/camera-daemon" "$build_dir/bin/rv1106_ai_ui" "$font_file" "$config_file" "$project_dir/scripts/run_demo.sh" "$project_dir/scripts/start.sh" "$project_dir/scripts/stress_test.sh"; do
