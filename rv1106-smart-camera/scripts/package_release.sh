...
mkdir -p "$stage/bin/fonts" "$stage/scripts"
...
for f in run_demo.sh start.sh stress_test.sh mode_ctl.sh; do install -m 755 "$project_dir/scripts/$f" "$stage/scripts/$f"; done
