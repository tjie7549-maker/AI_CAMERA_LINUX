...
bool ResultStorageClient::save(const QString &source, const QString &requestId) {
    return post(
        url_,
        QJsonObject{{QStringLiteral("source"), source}, {QStringLiteral("request_id"), requestId}},
        source, requestId);
}
...
bool ResultStorageClient::post(const QUrl &url, const QJsonObject &payload, const QString &source,
    ...
    reply_ = manager_->post(request, QJsonDocument(payload).toJson(QJsonDocument::Compact));
    timer_->start(10000);
...
void ResultStorageClient::recoverActiveEvent() {
    ...
    connect(request, &QNetworkReply::finished, this, [this, request]() {
        ...
        if (request->error() != QNetworkReply::NoError || !document.isObject()) {
            ...
            const QJsonArray events = document.object().value(QStringLiteral("events")).toArray();
