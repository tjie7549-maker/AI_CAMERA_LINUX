...
QString jsonString(const QJsonObject &object, const QString &key, const QString &defaultValue) {
    const QJsonValue value = object.value(key);
    return value.isString() ? value.toString() : defaultValue;
}
}  // namespace

AiResultClient::AiResultClient(const QString &serverIp, quint16 serverPort, QObject *parent)
    : QObject(parent),
      serverIp_(serverIp),
    ...
    connect(reconnectTimer_, &QTimer::timeout, this, &AiResultClient::connectToServer);
    ...
    connect(socket_, &QTcpSocket::connected, this, &AiResultClient::onConnected);
    connect(socket_, &QTcpSocket::disconnected, this, &AiResultClient::onDisconnected);
    connect(socket_, &QTcpSocket::readyRead, this, &AiResultClient::onReadyRead);
...
void AiResultClient::onReadyRead() {
    ...
    if (incoming.size() > kMaximumBufferSize - receiveBuffer_.size()) {
        ...
        emit errorOccurred(QStringLiteral("Receive buffer exceeded 64 KiB; discarded (%1 errors)")
    ...
    while ((newlineIndex = receiveBuffer_.indexOf('\n')) >= 0) {
        ...
        if (parseMessage(message, result, error)) {
            qInfo().noquote()
                << QStringLiteral(
                       "#AI result: scene=%1, people=%2, warning=%3, objects=%4, latency=%5 ms")
                       .arg(result.scene)
                       .arg(result.peopleCount)
                       .arg(result.warning ? QStringLiteral("true") : QStringLiteral("false"))
                       .arg(result.objects.isEmpty() ? QStringLiteral("none")
            ...
            const QString message =
                QStringLiteral("Invalid JSON #%1: %2").arg(invalidMessageCount_).arg(error);
...
bool AiResultClient::parseMessage(const QByteArray &message, AiResult &result, QString &error) {
    ...
    if (result.schemaVersion == 1 && (result.messageType.isEmpty() || result.cameraId.isEmpty())) {
    ...
    if (root.value(QStringLiteral("frame_timestamp_ns")).isDouble())
        result.frameTimestampNs =
            static_cast<quint64>(root.value(QStringLiteral("frame_timestamp_ns")).toDouble());
    if (root.value(QStringLiteral("server_latency_ms")).isDouble())
        result.serverLatencyMs =
            static_cast<qint64>(root.value(QStringLiteral("server_latency_ms")).toDouble());
    if (root.value(QStringLiteral("latency_ms")).isDouble())
        result.latencyMs = static_cast<qint64>(root.value(QStringLiteral("latency_ms")).toDouble());
    if (root.value(QStringLiteral("captured_at_ms")).isDouble())
        result.capturedAtMs =
            static_cast<qint64>(root.value(QStringLiteral("captured_at_ms")).toDouble());
    ...
    result.totalTokens =
        jsonInt(usage, QStringLiteral("total_tokens"), result.inputTokens + result.outputTokens);
    ...
    result.maxPeople = jsonInt(resultObject, QStringLiteral("max_people"), result.currentPeople);
    result.trackCount = jsonInt(resultObject, QStringLiteral("track_count"), 0);
    result.durationMs =
        static_cast<qint64>(resultObject.value(QStringLiteral("duration_ms")).toDouble(0));
    result.bestFrameId =
        static_cast<quint64>(resultObject.value(QStringLiteral("best_frame_id")).toDouble(0));
    result.eventState = jsonString(resultObject, QStringLiteral("event_state"), QString());
    result.cloudState = jsonString(resultObject, QStringLiteral("cloud_state"), QString());
    result.frameMatch = jsonString(resultObject, QStringLiteral("frame_match"),
                                   jsonString(root, QStringLiteral("frame_match"), QString()));
    result.peopleCount =
        jsonInt(resultObject, QStringLiteral("people_count"), result.currentPeople);
    result.warning = resultObject.value(QStringLiteral("warning")).toBool(false);
    result.warningReason =
        jsonString(resultObject, QStringLiteral("warning_reason"), result.warningReason);
    result.summary = jsonString(resultObject, QStringLiteral("summary"), result.summary);
    ...
    for (const QJsonValue &value : objects) {
        ...
        const QString name = jsonString(object, QStringLiteral("name"), QStringLiteral("Object"));
        const int count = jsonInt(object, QStringLiteral("count"), 1);
        result.objects.append(count > 1 ? QStringLiteral("%1 x%2").arg(name).arg(count) : name);
    }
    ...
    result.errorType = jsonString(root, QStringLiteral("error_type"),
                                  jsonString(errorObject, QStringLiteral("type"), QString()));
    result.errorMessage = jsonString(root, QStringLiteral("error_message"),
                                     jsonString(errorObject, QStringLiteral("message"), QString()));
    return true;
}
