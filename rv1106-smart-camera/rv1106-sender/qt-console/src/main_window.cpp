...
MainWindow::MainWindow(ManualRecognitionClient *manualClient,
                       ...
                       bool legacyAutoRecognitionEnabled, DaemonClient *daemonClient, QWidget *parent)
    : QWidget(parent),
      clockLabel_(nullptr),
      ...
      debugButton_(nullptr), daemonClient_(daemonClient),
      sceneValue_(nullptr),
...
void MainWindow::buildUi()
{
    ...
    videoHeading->addWidget(makeLabel(QStringLiteral("摄像头预览"), QStringLiteral("sectionTitle"), videoPanel));
    videoHeading->addStretch();
    debugButton_ = new QPushButton(QStringLiteral("参数 / 诊断"), videoPanel);
    debugButton_->setObjectName(QStringLiteral("debugButton")); debugButton_->setFixedSize(120, 34); debugButton_->setFocusPolicy(Qt::NoFocus);
    connect(debugButton_, &QPushButton::clicked, this, [this]() { CameraDebugDialog dialog(daemonClient_, this); dialog.exec(); });
    videoHeading->addWidget(debugButton_); videoLayout->addLayout(videoHeading);
