...
int main(void) {
    ...
    assert(tracker.count == 2 && tracker.tracks[0].track_id == 2 &&
           tracker.tracks[1].track_id == 3);
    assert(iou_tracker_iou(&two[0], &two[1]) == 0.0f);
    const TrackerDetection full =
        iou_tracker_from_letterbox(0.0f, 70.0f, 320.0f, 180.0f, 0.8f, 0, 320, 320, 384, 216);
    assert(full.x == 0.0f && full.y == 0.0f);
    assert(full.w == 1.0f && full.h == 1.0f);

    IouTracker crossing;
    iou_tracker_init(&crossing, 0.2f, 2, 1);
    TrackerDetection apart[] = {box(0.10f, 0.1f, 0.25f, 0.5f), box(0.65f, 0.1f, 0.25f, 0.5f)};
    TrackerDetection closer[] = {box(0.18f, 0.1f, 0.25f, 0.5f), box(0.57f, 0.1f, 0.25f, 0.5f)};
    TrackerDetection reversed[] = {box(0.49f, 0.1f, 0.25f, 0.5f), box(0.26f, 0.1f, 0.25f, 0.5f)};
