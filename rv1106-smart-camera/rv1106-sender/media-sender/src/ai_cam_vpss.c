...
int ai_cam_vpss_start(AiCamApp *app) {
    /* VPSS 是单路采集帧的分流点：显示、主/子编码、预览和人脸 ROI 共用此处输出。 */
    VPSS_GRP_ATTR_S grp_attr;
    VPSS_CHN_ATTR_S chn_attr;
    VPSS_CROP_INFO_S crop;
    int crop_size =
        app->config.vi_width < app->config.vi_height ? app->config.vi_width : app->config.vi_height;
    int crop_x;
    int crop_y;
    ...
    chn_attr.u32Depth = 2;
    chn_attr.u32FrameBufCnt = 3;
    ret = RK_MPI_VPSS_SetChnAttr(AI_CAM_VPSS_GRP, AI_CAM_VPSS_SUB_ENCODE_CHN, &chn_attr);
    if (ret != RK_SUCCESS)
        goto failed;
    memset(&crop, 0, sizeof(crop));
    crop.bEnable = RK_FALSE;
    ret = RK_MPI_VPSS_SetChnCrop(AI_CAM_VPSS_GRP, AI_CAM_VPSS_SUB_ENCODE_CHN, &crop);
    if (ret != RK_SUCCESS)
        goto failed;
    ...
    if (app->config.preview_shm_name) {
        ...
        chn_attr.u32Depth = 2;
        chn_attr.u32FrameBufCnt = 3;
        ret = RK_MPI_VPSS_SetChnAttr(AI_CAM_VPSS_GRP, AI_CAM_VPSS_PREVIEW_CHN, &chn_attr);
        if (ret != RK_SUCCESS)
            goto failed;
        memset(&crop, 0, sizeof(crop));
        crop.bEnable = RK_FALSE;
    ...
    if (app->config.enable_vo)
        printf("#VPSS display: crop [%d %d %d %d] -> scale %dx%d\n", crop_x, crop_y, crop_size,
               crop_size, app->config.vo_width, app->config.vo_height);
