...
/* H.264 编码模块：创建主/子 VENC 通道、绑定 VPSS，并在线程中将码流提交给 RTSP。 */
#include <string.h>
#include <time.h>
...
static RK_U8 *ai_cam_venc_payload(VENC_PACK_S *pack) {
    ...
    if (pack->u32Offset < pack->u32Len &&
        ai_cam_is_h264_start_code(data + pack->u32Offset, pack->u32Len - pack->u32Offset))
...
static int ai_cam_venc_start_channel(VENC_CHN channel, int width, int height, int source_fps,
...
int ai_cam_venc_start(AiCamApp *app) {
    int ret = ai_cam_venc_start_channel(AI_CAM_VENC_CHN, app->config.venc_width,
                                        ...
                                        app->config.venc_fps, app->config.venc_bitrate_kbps);
    if (ret != RK_SUCCESS)
        return ret;
    ...
    ret = ai_cam_venc_start_channel(AI_CAM_SUB_VENC_CHN, app->config.sub_venc_width,
                                    ...
                                    app->config.sub_venc_fps, app->config.sub_venc_bitrate_kbps);
    if (ret != RK_SUCCESS) {
...
static void *ai_cam_write_venc_stream(AiCamApp *app, VENC_CHN venc_channel,
                                      rtsp_session_handle rtsp_session, const char *output_path,
                                      bool stop_at_limit) {
    ...
    while (!ai_cam_is_stopping(app)) {
        ...
        for (index = 0; index < stream.u32PackCount && index < AI_CAM_MAX_VENC_PACKS; ++index) {
...
void *ai_cam_sub_venc_write_stream(void *arg) {
    ...
    return ai_cam_write_venc_stream(app, AI_CAM_SUB_VENC_CHN, app->rtsp_sub_session, NULL, false);
}
