...
真实结果见 `docs/test-report.md`；未真机验证项均保留为“待验证”。Qt 预览约 15 FPS、NPU 推理和手动曝光保持已经板端验证；8554 双码流、真实低照度指标、内核补丁刷入和长稳测试仍明确标记为待验证，不把模拟数据写成实测结论。
