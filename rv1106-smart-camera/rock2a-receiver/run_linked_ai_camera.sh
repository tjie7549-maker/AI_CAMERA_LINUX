...
usage() {
    cat <<EOF
Usage: $0 [--mode event|manual|auto] [legacy pipeline options]
...
case "$MODE" in event|manual|auto) ;; *) echo "Invalid mode: $MODE" >&2; exit 1 ;; esac
...
if [ "$MODE" = "event" ]; then
    AI_CAMERA_MODE=event setsid "$PROJECT_DIR/run_rock2a_supervisor.sh" &
