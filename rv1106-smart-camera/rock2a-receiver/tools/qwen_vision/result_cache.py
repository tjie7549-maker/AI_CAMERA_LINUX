...
def cache(
    ...
    try:
        ...
        _write(
            temporary / "metadata.json", json.dumps(metadata, ensure_ascii=False).encode() + b"\n"
        )
    ...
    entries = sorted(
        (item for item in base.iterdir() if item.is_dir()), key=lambda item: item.stat().st_mtime
    )
...
def save(root: Path, source: str, request_id: str) -> tuple[str, bool]:
    ...
    with index_path.open("a+", encoding="utf-8") as index:
        ...
        relative = (
            Path(source) / now.strftime("%Y-%m-%d") / (now.strftime("%H%M%S") + "_" + request_id)
        )
            ...
            json.dumps(
                {
                    ...
                    "scene": result.get("result", {}).get("scene"),
                    "warning": result.get("result", {}).get("warning"),
                    "saved_relative_path": str(relative),
                },
                ensure_ascii=False,
            )
            + "\n"
