#!/usr/bin/env python3
"""事件 HTTP API、事件驱动的云端识别与有界图像持久化。

接收本地 NPU 轨迹，维护事件状态，选择最佳帧，可选调用 Qwen，并将结果写入
SQLite 与供 Qt/TCP 回传进程读取的最新 JSON 文件。
"""
...
# 仅允许本服务生成的事件 ID 进入路径相关 API，避免目录穿越。
...
class EventServer(ThreadingHTTPServer):
    def __init__(
        ...
        self.engine = EventEngine(
            end_grace_seconds=end_grace,
            initial_delay_seconds=initial_delay,
            cooldown_seconds=cooldown,
            long_refresh_seconds=long_refresh,
        ...
        if event_id:
            ...
            if event:
                ...
                record = {
                    ...
                    "ended_at": (
                        event.last_seen_at if event.state in {"ended", "interrupted"} else None
                    ),
                    "duration_ms": max(0, event.last_seen_at - event.started_at),
                    "max_people": event.max_people,
                    "warning": event.warning,
                    "warning_reason": event.warning_reason,
                    "summary": event.summary,
                    "best_image_path": str(best.path) if best else None,
                    "best_frame_id": best.frame_id if best else 0,
                    "frame_match": "approximate",
                }
                self.store.upsert_event(record)
                for track in event.all_tracks.values():
                    self.store.upsert_track(event.event_id, track, event.last_seen_at)
        ...
        with self.lock:
            ...
            if event and best_track:
                ...
                if self.engine.consider_candidate(event, candidate):
                    ...
                    output.append(
                        self.engine._event_message(
                            event, "event.update", message["frame_id"], message["captured_at_ms"]
                        )
                    )
            for item in output:
                self.publish(item)
            ...
            with self.lock:
                ...
                if self.cloud_enabled and self.cloud_client:
                    for event in self.engine.due_cloud_events():
                        threading.Thread(
                            target=self._recognize_event, args=(event.event_id,), daemon=True
                        ).start()

    def _recognize_event(self, event_id: str) -> None:
        with self.lock:
            event = self.engine.events.get(event_id)
            if not event or not event.best:
                return
            ...
            try:
                ...
                self.publish(
                    self.engine._event_message(event, "event.update", frame_id, captured_at_ms)
                )
                return
        ...
        try:
            ...
            for attempt in range(self.max_retries + 1):
                ...
                try:
                    result = self.cloud_client.analyze_image_bytes(image_bytes, "image/jpeg")
                except Exception:
            ...
            document = envelope(
                ...
                model=result.get("model", "qwen3-vl-flash"),
        ...
        with self.lock:
            self.cloud_latency_ms = int(
                document.get("server_latency_ms", (time.monotonic() - started) * 1000)
            )
    ...
    def metrics(self) -> str:
        with self.lock:
            ...
            tracks = sum(
                len(self.engine.events[key].tracks) for key in self.engine.active_by_camera.values()
            )
        ...
        if self.frames.ring_dir and self.frames.ring_dir.is_dir():
            ...
            if metadata:
                try:
                    rtsp_frames = int(
                        json.loads(metadata[-1].read_text()).get("receiver_frame_id", 0)
                    )
                except (OSError, ValueError, json.JSONDecodeError):
                    pass
        values = {
            "ai_camera_events_total": self.store.count_events(),
            "ai_camera_active_events": active,
            "ai_camera_tracks_active": tracks,
            "ai_camera_npu_messages_total": messages_total,
            "ai_camera_npu_messages_invalid_total": messages_invalid,
            "ai_camera_cloud_requests_total": cloud_requests,
            "ai_camera_cloud_failures_total": cloud_failures,
            ...
            "ai_camera_best_frame_updates_total": best_updates,
            "ai_camera_storage_free_bytes": free,
    ...
    def do_GET(self) -> None:
        ...
        if path == "/health":
            ...
            self.send_json(
                200,
                {
                    ...
                    "cloud_enabled": self.server.cloud_enabled,
                },
            )
            return
        if path == "/metrics":
            data = self.server.metrics().encode()
        ...
        if path == "/events":
            ...
            if before and not before.isdigit():
                self.send_json(400, {"error": "invalid before"})
                return
        ...
        if len(parts) in {2, 3} and parts[0] == "events" and EVENT_ID_RE.fullmatch(parts[1]):
            ...
            if len(parts) == 3 and parts[2] == "image":
                ...
                try:
                    image_allowed = image.resolve().is_relative_to(allowed)
                except (OSError, ValueError):
                    image_allowed = False
                if not image_allowed or not image.is_file():
    ...
    def do_POST(self) -> None:
        if self.path == "/ingest":
            ...
            if payload is None:
                ...
                self.send_json(400, {"error": "invalid or too large json"})
                return
            accepted, output = self.server.ingest(payload)
            self.send_json(202 if accepted else 400, {"accepted": accepted, "events": output})
            return
        ...
        if (
            ...
            and parts[0] == "events"
            and parts[2] == "save"
            and EVENT_ID_RE.fullmatch(parts[1])
        ):
            if self.read_json(allow_empty=True) is None:
                self.send_json(400, {"error": "invalid or too large json"})
                return
            event = self.server.store.get_event(parts[1])
            source = Path(event.get("best_image_path") or "") if event else Path()
            allowed = (self.server.runtime / "event_best").resolve()
            try:
                source_allowed = source.resolve().is_relative_to(allowed)
            except (OSError, ValueError):
                source_allowed = False
            if not event or not source_allowed or not source.is_file():
                self.send_json(404, {"error": "event image not found"})
                return
            target = self.server.runtime / "saved_results" / "events" / parts[1]
            target.mkdir(parents=True, exist_ok=True)
            already_saved = (target / "event.json").is_file()
            shutil.copyfile(source, target / "best.jpg")
            atomic_json(target / "event.json", event)
            self.send_json(
                200,
                {
                    "success": True,
                    "already_saved": already_saved,
                    "saved_relative_path": str(target.relative_to(self.server.runtime)),
                },
            )
            return
        self.send_json(404, {"error": "not found"})
...
def main() -> int:
    ...
    parser.add_argument("--latest-image", type=Path, required=True)
    ...
    parser.add_argument("--initial-delay-seconds", type=int, default=3)
    ...
    server = EventServer(
        ...
        args.long_refresh_seconds,
        args.scene_change_threshold,
        args.end_grace_seconds,
        args.detection_stale_seconds,
