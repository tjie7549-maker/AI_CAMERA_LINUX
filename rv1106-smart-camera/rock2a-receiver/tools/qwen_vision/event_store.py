...
class EventStore:
    ...
    def _migrate(self) -> None:
        with self._lock, self.connection:
            ...
            version = int(
                self.connection.execute("SELECT version FROM schema_version LIMIT 1").fetchone()[0]
            )
    ...
    def close_open_events(self) -> None:
        ...
        with self._lock, self.connection:
            self.connection.execute(
                "UPDATE events SET state='interrupted', ended_at=?, duration_ms=? - started_at, updated_at=? WHERE state IN ('new','active','ending')",
                (now, now, now),
            )
    ...
    def upsert_event(self, event: dict[str, Any]) -> None:
        ...
        with self._lock, self.connection:
            self.connection.execute(
                """INSERT INTO events(event_id,camera_id,state,started_at,ended_at,duration_ms,max_people,warning,warning_reason,summary,best_image_path,best_frame_id,frame_match,created_at,updated_at)
    ...
    def upsert_track(self, event_id: str, track: dict[str, Any], now: int) -> None:
        with self._lock, self.connection:
            self.connection.execute(
                """INSERT INTO tracks(event_id,track_id,first_seen_at,last_seen_at,max_confidence) VALUES(?,?,?,?,?)
    ...
    def append_update(self, event_id: str, update_type: str, payload: dict[str, Any]) -> None:
        with self._lock, self.connection:
            self.connection.execute(
                "INSERT INTO event_updates(event_id,update_type,payload_json,created_at) VALUES(?,?,?,?)",
                (
                    event_id,
                    update_type,
    ...
    def record_recognition(self, document: dict[str, Any]) -> None:
        ...
        with self._lock, self.connection:
            self.connection.execute(
                """INSERT INTO recognitions(request_id,event_id,source,frame_id,backend,success,latency_ms,input_tokens,output_tokens,result_json,created_at)
                VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(request_id) DO NOTHING""",
                (
                    document["request_id"],
                    document.get("event_id"),
                    document.get("source", "cloud"),
                    document.get("frame_id", 0),
                    document.get("recognition_backend", "cloud"),
                    int(bool(document.get("success"))),
                    int(document.get("server_latency_ms", 0)),
                    int(usage.get("input_tokens", 0)),
                    int(usage.get("output_tokens", 0)),
                    json.dumps(document.get("result", {}), ensure_ascii=False),
                    int(time.time() * 1000),
                ),
            )

    def list_events(
        self, limit: int = 20, before: int | None = None, state: str | None = None
    ) -> list[dict[str, Any]]:
        ...
        with self._lock:
            rows = self.connection.execute(
                query + " ORDER BY started_at DESC LIMIT ?", (*values, max(1, min(limit, 100)))
            ).fetchall()
