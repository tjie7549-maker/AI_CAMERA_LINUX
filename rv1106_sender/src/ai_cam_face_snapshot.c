...
static int normalised_crop(float x, float y, float w, float h, int image_width,
			   int image_height, int *crop_x, int *crop_y, int *crop_width,
			   int *crop_height) {
	int left, top, right, bottom;

	if (!(x >= 0.0f && y >= 0.0f && w > 0.0f && h > 0.0f && x + w <= 1.0f &&
	      y + h <= 1.0f))
		return -1;
...
static int capture_nv12_roi(AiCamApp *app, float x, float y, float w, float h,
	...
	if (width <= 0 || height <= 0 || stride < width || (width & 1) || (height & 1))
		goto failed;
	if (normalised_crop(x, y, w, h, width, height, &crop_x, &crop_y, &crop_width,
	                    &crop_height) != 0)
		goto failed;
	...
	*result_bytes = (size_t)crop_width * (size_t)crop_height * 3 / 2;
	...
	for (row = 0; row < crop_height; row++)
		memcpy(copy + (size_t)row * crop_width,
		       mapped + (size_t)(crop_y + row) * stride + crop_x, crop_width);
